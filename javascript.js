...
.controller('mymCtrl', ['$scope', '$http', '$q', 'mymFactory', function ($scope, $http, $q, mymFactory) {

    // Import variables and functions from service    
    $scope.data = mymFactory.data;
	$scope.items = mymFactory.items;
	$scope.detailsItem = mymFactory.detailsItem;
	
    // initialize the service
    mymFactory.init($scope);

	// Show main view in the first column
	$scope.portalHelpers.showView('mymMain.html', 1);
	
	// This function gets called when user clicks an item in the list
	$scope.showDetails = function(item){
		// Make the item that user clicked available to the template
		$scope.detailsItem.value = item;		
		$scope.portalHelpers.showView('mymDetails.html', 2);
        
	};
	
}])
// Factory maintains the state of the widget
.factory('mymFactory', ['$http', '$rootScope', '$filter', '$q', function ($http, $rootScope, $filter, $q) {
...
.directive('mymDirectiveName', ['$http', function ($http) {
	return {
